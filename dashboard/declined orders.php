...
session_start();
$sql = mysqli_query($conn,"SELECT orders.o_id AS o_id, user.name AS name, orders.o_bottle AS o_bottle, orders.o_cost AS o_cost FROM user LEFT JOIN orders ON orders.user_id = user.id WHERE status = 2 ORDER BY o_id DESC");
