...
session_start();
$sql = mysqli_query($conn,"SELECT orders.o_id AS o_id, user.name AS name, orders.o_bottle AS o_bottle, orders.o_cost AS o_cost, orders.date_of_order AS date_of_order FROM user LEFT JOIN orders ON orders.user_id = user.id WHERE status = 4 ORDER BY o_id DESC");
...
<ul>
  ...
  <li><a style="color: blue;" href="admin_interface_delivery.php">Delivered</a></li>
